define([
    ...
    function onGameParametersUpdated() {
        ...
        gr.lib._warningAndError.on('click', function(e) {
            e.stopPropagation();
        });
        ...
        if (config.style.warningExitText) {
            gameUtils.setTextStyle(gr.lib._warningExitText, config.style.buttonStyle);
        }
        ...
        if (config.style.warningContinueText) {
            gameUtils.setTextStyle(gr.lib._warningContinueText, config.style.buttonStyle);
        }
        ...
        if (config.style.errorExitText) {
            gameUtils.setTextStyle(gr.lib._errorExitText, config.style.buttonStyle);
        }
        ...
        // 08 July 2019
        gameUtils.setTextStyle(gr.lib._winBoxErrorExitText, config.style.buttonStyle);
        ...
        if (error.errorCode === '00000' || error.errorCode === '66605') {
            ...
            if (window.loadedRequireArray) {
                for (var i = window.loadedRequireArray.length - 1; i >= 0; i--) {
                    require.undef(window.loadedRequireArray[i]);
                }
            }
        }
